...
5. iOS: no Mac on hand, but the path exists via cloud CI to TestFlight (details under Phase 9). Ignore iOS entirely until then, except an optional timeboxed TestFlight experiment after Phase 3 if I explicitly ask for it. (A weekly unsigned iOS build check already runs in GitHub Actions.)
6. Key commands:

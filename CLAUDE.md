...
5. iOS is untested and requires a Mac I do not have. Ignore iOS entirely until Phase 9.
6. Key commands:
...
- Phase 5: private beta, 4 weeks, 10 to 20 testers, APK shared via Telegram/WhatsApp plus the web demo link. Add a settings item that opens a prefilled email to abdulmoezshadi.90@gmail.com including app version and locale (no backend feedback path). Weekly beta builds, git tag per build. Capture the list of foods testers search for and do not find; it is the Phase 8 backlog.
...
- Phase 9 (H2 2027, decide with real data, not now): barcode scanner (only once verified packaged DB is large enough that scans usually hit), backend/accounts (only if users demand sync; AppState isolation makes it swappable), iOS, monetization (default: stay free; realistic future paths are local brand sponsorship, licensing the verified DB to clinics/dietitians, premium unlock via Libyan payment rails like Sadad with an own built activation code system; never ads).
...
2. Master data source is a spreadsheet I maintain, plus a folder of label photos named to match food IDs. food_db.dart is synced FROM it, never the other way.
